...
namespace posty::handlers::articles_slug::put {
...
userver::formats::json::Value Handler::HandleRequestJsonThrow(
    ...
    } catch (const posty::utils::error::ValidationException& ex) {
    ...
    try {
        const auto new_slug =
            update_request.title
                ? std::make_optional<std::string>(posty::utils::slug::Slugify(*update_request.title))
                : std::nullopt;
        const auto res = GetPg().Execute(
            userver::storages::postgres::ClusterHostType::kMaster,
            posty::sql::kUpdateArticleBySlug,
            slug,
            user_id,
            update_request.title,
            new_slug,
        ...
        if (constraint == "uniq_slug") {
            request.SetResponseStatus(userver::server::http::HttpStatus::kUnprocessableEntity);
            return posty::utils::error::MakeError("slug", "already exists");
        }
        throw;
    }
    const auto res = GetPg().Execute(
        userver::storages::postgres::ClusterHostType::kMaster,
        posty::sql::kGetArticleWithAuthorProfile,
        article_id,
        user_id
    );

    userver::formats::json::ValueBuilder builder;
    builder["article"] = res.AsSingleRow<posty::models::TaggedArticleWithProfile>();
